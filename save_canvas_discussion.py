#######################
# Fork to produce only a single HTML file with all replies in that one file
    ...
    for student in data['participants']:
        ...
        for post in data['view']:
            if 'user_id' in post.keys():
                ...
                if post['user_id'] == id:
                    write_post_to_file(os.path.dirname(fname), name, id, post['message'], post['updated_at'],
                                       id_to_name, os.path.basename(fname).split('.')[0], replies)
...
def usage():
    ...
    OUT(
        '   If the script is run with the filename that you saved from the previous step, it will save posts for each student\n')
